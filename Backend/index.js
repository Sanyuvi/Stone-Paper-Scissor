...
app.put("/playgame/:id", async (req, res) => {
  ...
  const {
    ...
    player2_choice,
    round,
  } = req.body; // Get new round data from the request body
  ...
  try {
    ...
    const updatedRounds = [
      ...roundData,
      { player1_choice, player2_choice, player1_score, player2_score, round },
    ];

    const totalPlayerScore1 = updatedRounds.reduce(
      (pre, crr) => pre + crr.player1_score,
      0
    );
    const totalPlayerScore2 = updatedRounds.reduce(
      (pre, crr) => pre + crr.player2_score,
      0
    );
