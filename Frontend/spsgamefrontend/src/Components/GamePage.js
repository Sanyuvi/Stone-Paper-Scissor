...
const GamePage = () => {
  ...
  const intialChoiceValue = {
    round: 0,
    player1_choice: "fa-solid fa-question-circle",
    player2_choice: "fa-solid fa-question-circle",
    player1_score: 0,
    player2_score: 0,
    player1color: "text-muted",
    player2color: "text-muted",
    player1ChoiceName: "Choose option!",
    player2ChoiceName: "Choose option!",
  };
  const [startGame, dispatchStartGame] = useReducer(
    gameReducer.gameStartReducer,
    { visible: true, userChoiceVisible: false, text: "Play" }
  );
  const [choices, dispatchChoices] = useReducer(
    gameReducer.gameChoiceReducer,
    intialChoiceValue
  );
  ...
  const selectedChoice = async (choicesID) => {
    ...
    if (
      (choiceDetails[0].id === 1 && computerDetail.id === 3) ||
      (choiceDetails[0].id === 3 && computerDetail.id === 2) ||
      (choiceDetails[0].id === 2 && computerDetail.id === 1)
    ) {
      ...
      <div className="container-fluid bg-secondary-subtle vh-100 ">
        ...
        <div className="row m-5">
          ...
          <div className="col-6 align-self-center">
            ...
            {finalWinner ? (
              <p className="m-0 tada">
                Game Over !!! <br /> Final Winner: {finalWinner}
              </p>
            ) : (
              <div className="row d-flex justify-content-center">
                <div className="col-6">
                  <i
                    className={`${choices.player1_choice} shadow p-2 rounded-4 border ${choices.player1color}`}
                    style={{ fontSize: 70 }}
                  ></i>
                ...
                <div className="col-6">
                  <i
                    className={`${choices.player2_choice} shadow p-2 rounded-4 border ${choices.player2color}`}
                    style={{ fontSize: 70 }}
                  ></i>
